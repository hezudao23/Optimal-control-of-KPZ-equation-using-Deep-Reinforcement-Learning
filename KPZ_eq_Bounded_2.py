# -*- coding: utf-8 -*-
"""
Created on Mon Mar 26 10:55:56 2018
...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  1 17:21:32 2018
...
#30 * 30 works [dim + 1, dim + 10m dim + 10, dim ]
#30 * 50 works [dim + 1, dim + 10m dim + 10, dim ]
#50 * 10 works [dim + 1, dim  + 20, dim + 10, dim + 1]
#100 * 10, it seems we need deeper network to have it works well.num_hidden = [dim +1, dim + 25, dim+50, dim + 25, dim]
#100 * 10 also works num_hidden = [dim +1, dim + 25, dim+50, dim + 25, dim] 
#100 * 10 works really well with [dim +1, dim + 50, dim+100, dim + 50, dim]
dim = 30 # 30 works
""" Extra 1 is for time stamp"""
total_time = 1
num_time_interval = 10 # this cannot be very large 10# 20 works # 30 works
delta_time = total_time / num_time_interval
nu = 1
lda = 0.01
#num_hidden = [dim+1, dim + 10, dim + 10, dim]
num_hidden = [dim +1, dim + 10, dim+20, dim + 10, dim] # i change it from dm + 1, dim + 10, dim + 10 dim
''' This is the lambda'''
#####
'''Some constant for control signal '''
Mg = tf.constant([2.5]) # 2.5
T_0 = tf.constant([3.0]) # 3 
#lr = 1e-3
####
...
#def control(u):
#    C_1 = tf.div(2 * Mg, 1 + tf.exp( -1 * tf.div(u, Mg)))  
#    return tf.subtract(C_1, Mg)
...
#            b = tf.get_variable('Bias', [1, num_hidden[i]], tf.float32,  tf.random_normal_initializer(stddev = 1/np.sqrt(num_hidden[i - 1] + num_hidden[i])))
#            if i < len(num_hidden) - 1:
#                input_ = tf.nn.relu(tf.add(tf.matmul(input_, W), b))
#            else:
...
#            W = tf.get_variable('Matrix', [num_hidden[i-1], num_hidden[i]], tf.float32, tf.random_normal_initializer(stddev = 0.1)) # 0.01
            ...
            if i < len(num_hidden) - 1:
                input_ = tf.nn.sigmoid(tf.add(tf.matmul(input_, W), b))
            else:
                input_ = tf.add(tf.matmul(input_, W), b)
...
noise = tf.placeholder(tf.float32, shape = [num_time_interval, dim])
noise_sample = []
...
#    grad_y = subnetwork(z, name)
    ...
    c_signal = -0.25/lda * tf.multiply(tf.square(z), hessian_part) 
...
for idx in range(128):
    ...
    noise_sample.append(np.random.randn(num_time_interval, dim) * np.sqrt(delta_time))



for i in range(20000 + 1):
    
    seed = int(np.random.uniform(0, 128 - 1, 1))
        
    #noise_cur = np.random.randn(num_time_interval, dim) * np.sqrt(delta_time)
    
    noise_cur = noise_sample[seed]
    
    train_step.run(feed_dict = {noise: noise_cur})
    
    if i % 10 == 0:
        [loss_res, y_init_res] = sess.run([loss, y_init], feed_dict = {noise: noise_cur})
